...
const perTaskTimeoutSec = parseInt(flag("timeout", "90"), 10) || 90;
const maxRetries = parseInt(flag("retries", "1"), 10) || 1;
const taskFilter = flag("tasks") ? new Set(flag("tasks").split(",")) : null;
...
const TASKS: Task[] = [
	...
	{
		name: "encode",
		prompt: "Compute the Model2Vec embedding vector for the text 'robot bridge initialization'. Use an encode tool.",
...
function killProcessTree(pid: number) {
	try {
		const children = fs
			...
			.split(/\s+/)
			.filter(Boolean);
		for (const c of children) killProcessTree(parseInt(c, 10));
	} catch {}
	try {
...
async function runOne(task: Task, variant: "omp" | "omp-semble"): Promise<RunResult> {
	...
	for (let attempt = 0; attempt <= maxRetries; attempt++) {
		const args =
			variant === "omp"
				? ["--tools", "read,bash,edit,write,find,search,ast_grep,lsp,task,todo,web_search,browser,ask"]
				: [];
		...
		try {
			...
			for (const f of files.sort().reverse()) {
				...
				if (stat.mtimeMs >= startTs - 5000) {
					// within 5s before start
