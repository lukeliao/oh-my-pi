...
import {
	...
	HIDDEN_TOOL_NAMES,
	normalizeToolName,
} from "../../packages/coding-agent/src/tools/builtin-names";
...
/**
 * Tool registry for zero-call analysis. Builtins come from the live source of
 * truth (packages/coding-agent/src/tools/builtin-names.ts); custom/MCP tools
 * enumerate the CURRENT mount surface only (our semble tools, advisor, OKF,
 * currently-mounted MCP servers). Historical aliases (pre-v16 tool names, old
 * mcp__ prefixes) are NOT registered — observed legacy names are reported
 * separately as `observed-legacy` so the zero-call denominator reflects the
 * current surface only. Update the custom block when mounts change.
 */
...
function printScanReport(res: AuditResult): void {
	...
	// Observed names are normalized (legacy aliases search→grep, find→glob)
