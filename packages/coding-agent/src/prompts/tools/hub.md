...
Every call MUST set `op` explicitly — launching a process is `op:"start"` (a call carrying `application` but no `op` is repaired to `start`).
...
- **`wait`** with `name` blocks until readiness/exit/`pattern` or `timeout` (seconds; `timeoutMs` in milliseconds is accepted as a fallback). `pattern` is a JavaScript `RegExp` compiled with the `u` flag (no inline modifiers such as `(?i)`). A `pattern` wait also returns when the process exits without ever printing it — check the reported state before assuming a match.
